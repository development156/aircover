...
6. **Verify.** Run `pnpm turbo run typecheck lint test --filter=...[origin/main]`. Live-database tests reporting "skipped" is normal in the cloud sandbox (no `.env` on purpose) — never try to un-skip them or invent credentials.
