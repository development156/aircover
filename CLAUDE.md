...
`pnpm gate` = `turbo run typecheck lint test && turbo run test:smoke && prettier --check .` — one command, because the two halves that used to sit outside it were both silently red for months. `format:check` is a ROOT script outside turbo, so a green turbo count says nothing about formatting. And `turbo test` runs VITEST ONLY: the Playwright suite sat outside the gate for twenty runs while `golden-path` was failing the whole time, because a product change turned "Create post" from a `<button>` into a `<Link>` and nothing was watching. MEASURED 2026-08-22: `playwright test --list` reports **102 tests in 32 files** and `--grep @smoke` reports **91**. (It read 76/19 and 67 on 2026-08-20; the lanes merged into `wt-integrate` since then brought the rest, and a stale number here is the same defect as a stale number on a screen.) The eleven outside the tag are deliberate and each says why in its own header — `assets.spec.ts` uploads real bytes to storage, `design-audit.spec.ts` is a screenshot tool. The gate runs the 91, not "every one of them".
