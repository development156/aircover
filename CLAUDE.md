...
`pnpm gate` = `turbo run typecheck lint test && turbo run test:smoke && prettier --check .` — one command, because the two halves that used to sit outside it were both silently red for months. `format:check` is a ROOT script outside turbo, so a green turbo count says nothing about formatting. And `turbo test` runs VITEST ONLY: the Playwright suite sat outside the gate for twenty runs while `golden-path` was failing the whole time, because a product change turned "Create post" from a `<button>` into a `<Link>` and nothing was watching. MEASURED 2026-09-06 from the CI reports of run 34021699957 on `973c6534`: `--grep @smoke` is **122 tests in 38 files**, and the run PASSED all 122 with 0 skipped, in three shards, none near the 60-minute limit. (It read 118 in 37 files on 2026-08-26 and 277 total in 72 files by `playwright test --list`; there are 73 spec files now, and the local `--list` is refused on a laptop whose env names production, so the total is not re-measured here.) **The smoke leg runs on CI against STAGING, and only there:** `.github/workflows/gate.yml`'s `smoke` job is dispatched by hand with `ack_target=yoxmzwkxweasfaahhvpj`, reads the four `E2E_SUPABASE_*` repository secrets (staging) plus the two Clerk ones (the development instance, because the fixture's `+clerk_test` users only exist there), refuses production outright, checks each secret's shape, signs in to the database once (`scripts/smoke-db-probe.mjs`) before spending an hour, builds the app, runs it with `next start`, and refuses a report that skipped a test or holds none (`scripts/smoke-skips.mjs`). It took nine dispatches on 2026-09-05/06 to reach green, and every one of the blockers was an environment fact a person had to fix (production secrets, a re-saved URL, staging not trusting Clerk, a wrong database password): when the guard steps refuse, REPORT the sentence they print, never inline a key, relax a guard or un-skip a spec. In the claude.ai/code remote sandbox the leg cannot run at all: Playwright's Chromium cannot complete any outbound HTTPS request there (MEASURED, REQUESTS §25), so run it on CI. The 155 outside the tag are deliberate and each says why in its own header — `assets.spec.ts` uploads real bytes to storage, `design-audit.spec.ts` is a screenshot tool, `onboarding-build.spec.ts` and `onboarding-money-guard.spec.ts` each drive eight screens and mint a Clerk user. The gate runs the 122, not "every one of them". **A stale number here is the same defect as a stale number on a screen** — re-measure it in the same commit that moves it.
