...
/**
 * `public.resolve_brand_memory`, EXECUTED.
 *
 * ── WHY THIS FILE EXISTS BESIDE brand_memory.test.ts ────────────────────────
 * The live file carries these 28 claims under `describe.skipIf(!hasRlsEnv)`, and
 * there is no configuration in which they run (see connections.pglite.test.ts
 * for the two-way trap). It last ran live on 2026-07-19 against the v1 function;
 * the v2 rewrite (20260812000001) has never been executed by it. The function is
 * SECURITY DEFINER, so `brand_memory`'s read-only tenant policy is bypassed
 * inside it and the membership + role allowlist in its body is the SOLE write
 * boundary of the only client-reachable write into the Brand Brain.
 *
 * Every claim is kept. Three are retargeted for the harness and say so inline:
 *   · "omitted" versus "explicit null" arguments are distinguished by building
 *     the call with NAMED arguments for exactly the keys the test supplies, which
 *     is what PostgREST does with a JSON body;
 *   · the two "concurrent" tests run their four calls one after another. PGlite
 *     is one connection, so nothing here can contend for the advisory lock. What
 *     they still prove is the ORDERING contract (contiguous versions, one active
 *     row, identical resubmits collapsing to one); the race itself is only
 *     provable live, and that is stated rather than pretended;
 *   · error CODES come off the PGlite error object, which carries SQLSTATE
 *     exactly as PostgREST relays it.
 *
 * Mutation that proves the boundary: in 20260812000001, change
 * `v_role not in ('owner', 'editor', 'approver')` to `('nobody')` and the viewer
 * test goes red; delete the `if not found then raise NOT_A_MEMBER` branch and
 * the cross-tenant test goes red with B's payload active in A.
 */
...
type Outcome = { data: ResolveResult | null; error: PgError | null }
type StoredRow = {
  id: string
  version: number
  status: string
...
async function asClaimsCommitting<T>(
  db: PGlite,
...
describe('public.resolve_brand_memory', () => {
  let db: PGlite
  let seq = 0

  /**
   * Named arguments for exactly the keys supplied, so an OMITTED p_source takes
   * the SQL default and an explicit null is passed as null — the distinction the
   * "explicit null source" test exists to pin.
   */
  async function resolve(sub: string, args: Args): Promise<Outcome> {
    const named: string[] = ['p_workspace_id => $1::uuid']
    const params: unknown[] = [args.workspaceId]
    if ('payload' in args) {
      params.push(args.payload === null ? null : JSON.stringify(args.payload))
      named.push(`p_payload => $${params.length}::jsonb`)
    }
    if ('source' in args) {
      params.push(args.source)
      named.push(`p_source => $${params.length}::text`)
    }
    if ('expectedVersion' in args) {
      params.push(args.expectedVersion)
      named.push(`p_expected_version => $${params.length}::int`)
    }
    try {
      const r = await asClaimsCommitting(db, { sub, role: 'authenticated' }, (tx) =>
        tx.query<{ r: ResolveResult }>(
          `select public.resolve_brand_memory(${named.join(', ')}) as r`,
          params,
        ),
      )
  ...
  /** Ground truth read as superuser — never trust the RPC's own echo. */
  async function rows(ws: string): Promise<StoredRow[]> {
    return (
      await db.query<StoredRow>(
        `select id, version, status, source, payload, created_by
           from brand_memory where workspace_id = $1 order by version`,
        [ws],
      )
    ).rows
  }
  ...
  it('persists v1 as active, attributed to the caller, in the frozen envelope shape', async () => {
    ...
    // the envelope must parse as the shared contract apps/web consumes
  ...
  it('the member can read their own brain back through RLS (t_select)', async () => {
    ...
    const got = await asMember(db, sub, (tx) =>
      probe<{ version: number; status: string }>(
        tx,
        `select version, status from brand_memory where workspace_id = $1`,
        [ws],
      ),
    )
    expect('rows' in got ? got.rows : got.denied).toEqual([{ version: 1, status: 'active' }])
  })
  ...
  it('both versions stay readable by the member after a supersede', async () => {
    ...
    const got = await asMember(db, sub, (tx) =>
      probe<{ version: number; status: string }>(
        tx,
        `select version, status from brand_memory where workspace_id = $1 order by version`,
        [ws],
      ),
    )
  ...
  it('version numbering skips over draft rows (max is taken across all statuses)', async () => {
    ...
    await db.query(
      `insert into brand_memory (workspace_id, version, status, payload, source)
       values ($1, 5, 'draft', $2::jsonb, 'manual')`,
      [ws, JSON.stringify(brandPayload('draft'))],
    )
  ...
  // ── concurrency (SERIALISED here — see the header) ────────────────────────
  it('four distinct resolves produce contiguous versions and one active row', async () => {
    const { ws, sub } = await freshWorkspace()
    const results: Outcome[] = []
    for (const t of ['w', 'x', 'y', 'z']) {
      results.push(await resolve(sub, { workspaceId: ws, payload: brandPayload(t) }))
    }
  ...
  it('four identical resolves collapse to a single version', async () => {
    const { ws, sub } = await freshWorkspace()
    const payload = brandPayload('rage-click')
    const results: Outcome[] = []
    for (let i = 0; i < 4; i += 1) results.push(await resolve(sub, { workspaceId: ws, payload }))
  ...
  it("cross-tenant: a member of B cannot write — or disturb — tenant A's brain", async () => {
    ...
    const peek = await asMember(db, b.sub, (tx) =>
      probe<{ id: string }>(tx, `select id from brand_memory where workspace_id = $1`, [a.ws]),
    )
    expect('rows' in peek ? peek.rows : peek.denied).toHaveLength(0)
  })
  ...
  it('signed-out anon is denied at the permission layer, not inside the function', async () => {
    ...
    try {
      await asRole(db, 'anon', {}, (tx) =>
        tx.query(`select public.resolve_brand_memory($1::uuid, $2::jsonb)`, [
          ws,
          JSON.stringify(brandPayload('anon')),
        ]),
      )
    } catch (e) {
      const err = e as { message?: string; code?: string }
      error = { message: String(err.message ?? e), code: err.code }
    }
