...
export const CARDS = [
  ...
  {
    roadmap: null,
    title: 'Tell people their connection is about to expire, instead of only showing them',
    plain:
      'The seven-day warning exists, but only on the Connections page — you have to go and look. Nothing emails, texts or notifies anybody. Why it matters: the whole point of the warning is to reach someone who is NOT looking at the app; a customer who last signed in three weeks ago finds out their Instagram stopped working when a post fails to go out.',
    technical:
      'doc 13 §8 step 3 is "warn the customer in-app AND by email, with a one-click reconnect". The in-app half shipped and is mounted: `apps/web/src/lib/connections/health.ts` derives the T-7 verdict AT READ TIME from `connections.expires_at` (idempotent by construction — it cannot fire twice and needs no have-we-warned table), and `ConnectionHealthBanner` + `connection-row` render it with channel-specific copy from `healthMessage()`. The apps/jobs half is done and proven: the reconcile pass writes `expires_at` every time it reads an account, held by `mutations/dead-account.mjs`. What does not exist is any notification rail at all — no Resend send, no digest job, no per-workspace notification preference, no sent-log to dedupe against, no unsubscribe. Deliberately NOT bolted onto the reconcile sweep: that is a read-and-reconcile pass and has no business sending mail, and a sweep that sends on every tick warns the same person every five minutes. `health.ts` also has no test file of its own, which should be fixed by whoever builds the send.',
  },
  {
    roadmap: null,
    title: 'Make a health check that failed completely answer like a failure',
    plain:
      'The five-minute health check now says in one word whether it went fine, partly wrong, or wholly wrong — but the request it answers still reports success either way. Why it matters: the monitoring dashboard reads the status, not the words, so a run where every single account check failed still shows up as a green tick.',
    technical:
      "apps/jobs is done: `ReconcileReport` carries `outcome: 'clean' | 'degraded' | 'failed'`, per-half failure counts, and a classified `failures[]` — codes and counts only, never a message and never a row id, because `run-sweeps.ts` returns this report whole on a public URL and a database error message routinely carries a host. apps/web owns the last step: `runCronSweeps` computes `ok` from `isError()` alone, so a report that says `outcome: 'failed'` still yields HTTP 200 and `ok: true`. It should fold `reconcile.outcome === 'failed'` into `ok` (and decide what `degraded` deserves), and pass `reconcileSweepDeps({ onFailure })` through to `reportServerError` so the real cause reaches Sentry — the hook exists, is typed, and is currently wired to nothing.",
  },
  {
    roadmap: null,
    title: "Confirm the platform really tells us when an account's access expires",
    plain:
      'The expiry warning is computed from a date the platform sends us. We store it, we read it back, and the banner is built and mounted — but nobody has confirmed the platform actually puts that date in the particular reply we read it from. Why it matters: if it does not, every piece of the warning works perfectly and the warning never appears.',
    technical:
      "The chain is `listAccounts` → `a.tokenExpiresAt` → `AccountFacts.tokenExpiresAt` → `connections.expires_at` → `daysUntil()` → the T-7 banner. Every link but the FIRST is now executed in a test. `tokenExpiresAt` is optional on `ZernioAccount`, and doc 13 §2.5 records it on the CONNECT response (`expires_in: 5184000`, exactly 60 days) — not demonstrably on `GET /accounts`, which is what the sweep reads. If it is absent there, the store's `coalesce` correctly keeps whatever connect wrote, and that value then quietly ages out sixty days later with no second source to refresh it. Close this by reading one real /accounts response for a live profile and recording the field in doc 13, or by having the OAuth return route persist the connect expiry as the fallback.",
  },
  {
    roadmap: null,
    title: 'Re-run the new publishing and health checks against the real database',
    plain:
      'Two rules that stop a post going out twice, and stop a dead account being used, are now proven by running them on a real database engine bundled into the test — but against a hand-written copy of the tables rather than the real ones. Why it matters: the rules are proven, the columns are not; a table that has since gained a constraint or lost a default would still pass.',
    technical:
      '`src/publish/lease.pglite.test.ts` and `src/reconcile/store.pglite.test.ts` run on PGlite (Postgres compiled to WASM, in-process, a devDependency of apps/jobs) with a minimal DDL covering only the columns each statement touches. That is deliberate and it is what made the checks possible at all: `tests/helpers/forbidden-target.ts` bans the one Supabase project by ref, so the live suites cannot run here and a `describe.skipIf` suite would have fed a mutation spec zero tests — which the harness scores as SURVIVED, i.e. proof of nothing. Schema drift therefore stays uncovered until either a branch database exists (run the same assertions through `tests/*.integration.test.ts`, which already skip cleanly) or the real DDL is loaded into PGlite from `packages/db/supabase/migrations`.',
  },
  {
    roadmap: null,
    title: 'Close the gap where a crashed publish could post the same thing twice',
    plain:
      'Recovering a post whose publisher was killed mid-flight now works — it used to be abandoned forever. The recovery re-sends it, and in one narrow case the original had already gone out: if the process died in the instant between the platform accepting the post and us writing it down, nothing we store knows it exists. Why it matters: the customer sees the same post twice, and this is the one failure the whole publishing design is built to prevent.',
    technical:
  ...
  {
    roadmap: null,
    title: 'Make a repeated channel harmless everywhere instead of fixing it one screen at a time',
    plain:
      'The list of channels a post is aimed at can hold the same channel twice, and every screen that reads it assumes that cannot happen. Three separate faults have shipped from this in two days: a schedule warning that promised a post would go out when nothing could receive it, a sentence that named the same account twice, and two identical Publish buttons on one post. Why it matters: each fix corrected the one screen it was found on and the fault simply moved to the next screen, so the customer keeps meeting a new version of the same bug.',
    technical:
      "`posts.channels` is a Postgres `text[]` with no unique constraint, typed `Channel[]` by `PostSchema` and handed to every consumer untouched \u2014 the planner and the editor never dedupe it. Every consumer nonetheless reads it as a SET. Three shipped defects on 2026-08-09/10, one per failure mode. (1) COUNT: `scheduleGapNote` derived \"is any channel still connected\" as `unconnectedNames.length >= totalChannels`, which is false for `['linkedin','linkedin']`, so the picker promised delivery when nothing could receive the post. (2) NAMES: the undeduplicated name list rendered \"LinkedIn and LinkedIn aren't connected\" \u2014 two accounts named where one was broken, on a plural verb the count had no right to; fixed in `5b84e34` inside `unconnectedFrom`. (3) REACT KEYS: `PublishNow` derived its live list from the raw array while only the warning went through `unconnectedFrom`, so a repeated CONNECTED channel rendered two identical Publish buttons under one React key; shipped to production, fixed in `e31fee1` by deduplicating the shared input. Each fix deduplicated at ONE site and the fault moved to a sibling, and the tests passed every time because each asserted the branch that had just been fixed rather than the sibling that had not.\n\nPROPOSAL \u2014 make the three classes unreachable at typecheck time instead of deduplicating at N call sites. CONSIDERED AND REJECTED: typing `Post.channels` as a `ReadonlySet<Channel>` via a `PostSchema` transform. It closes the read side cleanly, but `savePost(postId, patch: unknown)` takes an untyped patch BY DESIGN \u2014 it is a server action receiving untrusted input and narrows with zod inside. A `Set` reaching it therefore compiles, serializes to `{}`, and fails the zod parse at runtime with no compile-time catch: three read-side classes closed in exchange for one new write-side class. `useAutosave`'s `sameDraft` also compares channels by index and would silently need set semantics.\n\nRECOMMENDED: a branded distinct array in `packages/shared` \u2014 `type DistinctChannels = readonly Channel[] & { readonly __distinct: unique symbol }`, constructible only through one `channelSet()` helper, with `PostSchema.channels` becoming `z.array(ChannelSchema).transform(channelSet)`. It serializes as an array, so the wire format and the deliberately untyped write boundary are both unchanged; `sameDraft`'s index comparison keeps working; JavaScript insertion order is preserved, so the \"DISTINCT, in first-seen order\" contract `unconnectedFrom` already documents survives; and any consumer declaring `DistinctChannels` rejects a raw `Channel[]` at compile time. That last property is what closes all three classes at once rather than one at a time \u2014 a count, a rendered name list and a React key are three different symptoms of one type mismatch, and only the type can close them together.\n\nTHE DRAFT PATH IS PART OF THE BOUNDARY, not downstream of it. `PostDraft.channels` is a plain `Channel[]` seeded by `toDraft` straight from `post.channels`, and the editor passes `draft.channels` — never `post.channels` — into `BottomBar`, which is what feeds `PublishNow`, the site of defect (3). Assigning a branded array into a plain `Channel[]` field widens silently and drops the guarantee, so branding `PostSchema` alone would leave the exact component this week's fix touched still reading an unbranded list. `PostDraft.channels` has to carry the brand too, with `channelSet()` the only constructor on both the read path and the edit path. The picker is NOT the source: `ChannelPicker`'s toggle rebuilds the selection from `ChannelSchema.options`, so it cannot add a repeat and in fact clears one on the first toggle. The draft carries duplicates rather than creating them, which is the harder case to test — the fault is invisible until someone opens a row that already has one.\n\nCOST, stated up front. `Post.channels` is a frozen contract, so this is a `[contract]`-prefixed PR touching roughly 30 call sites across apps/web and packages/mesh. It also does NOT clean rows already stored: duplicates already in Postgres stay duplicated, and the read boundary only stops them mattering. A database-level guard would be wt-db's lane and is deliberately not part of this.",
  },
  {
    roadmap: null,
    title: 'Make the formatting check part of the gate that runs before anything ships',
    plain:
      'The check that keeps every file formatted the same way was not part of the set of checks run before shipping. Work could pass every check, be committed, and then be reformatted the moment anyone next opened those files \u2014 which cost an extra deploy today. The formatting check now runs alongside the others. Why it matters: a green result that is silent about something it never looked at gets trusted more than it has earned.',
    technical:
      "`format:check` (`prettier --check .`) is a ROOT package.json script and sits OUTSIDE the turbo pipeline, so it was in neither the eight-package fan-out nor the Stop hook. \"typecheck 8/8, lint 8/8, test 14/14\" was true every time and silent about formatting. Observed 2026-08-10 while merging wt-metrics and wt-inbox3: format:check exited 1 on 11 files while 24/24 turbo tasks were green, and the PostToolUse prettier hook then rewrote each file on contact, producing a commit and a deploy whose only content was reformatting.\n\nFIX: appended `&& pnpm format:check` to the gate string at every site that states it \u2014 the ship and fix commands, the sahoda-ship skill, docs 09, 10 and 12, the root CLAUDE.md commands line, and the Stop hook (whose verbatim mirror in doc 12 was updated in the same change). Verified once end to end: 24/24 turbo tasks plus a clean prettier run.\n\nDeliberately NOT collapsed into a single `pnpm gate` script, even though that would remove six copies of one string. `ops-classify.mjs` derives QA suites by regex over the LITERAL Bash command string, so a command named `gate` matches no runner pattern and would have silently stopped recording QA runs altogether \u2014 the gates strip would then have shown every suite as unseen. Keeping the words `typecheck`, `lint` and `test` inside the command preserves classification exactly as it was. One clean name that breaks the QA record is worse than six strings that classify correctly.\n\nTHREE THINGS THIS LEAVES, none of them fixed here. (1) Formatting cannot be recorded as a QA suite of its own: `ops_qa_runs.suite` carries `check (suite in ('typecheck','lint','unit','rls','smoke','e2e','manual'))` in an APPLIED migration, and only wt-db may add to that list. (2) A formatting-only failure is therefore recorded as a single red row labelled `unit` \u2014 `verdictFor` reads prettier's \"Code style issues found\" and pnpm's `ELIFECYCLE` as failure evidence, and `suiteFor` labels an aggregate failure with the most substantive suite present. Collapsing an aggregate failure to one row is pre-existing and deliberate, but it now has one more way to fire and the label is wrong for this case; a red run blocks a card from Done, so the wrong label has teeth. (3) The Stop hook still runs typecheck and test WITHOUT lint while the ship gate runs all three. That divergence predates this change and is untouched by it.",
  },
  {
    roadmap: null,
    title: 'Stop pointing every working session at the live customer database',
    plain:
      'The tool an assistant uses to look inside the database while working is wired to the live customer database, in every session, and nobody chooses that per task — it is simply what the project hands out. A rehearsal copy now exists and nothing points at it. Why it matters: routine inspection happens against real customer records, and the only thing standing between a session and a change to them is a single setting in a file anyone can edit.',
    technical:
      "P0, filed 2026-08-11 by DIVAS as a PROCESS defect rather than a code defect — nothing here is broken by its own lights, which is why it survived. `.mcp.json` pins the Supabase MCP server to `--read-only --project-ref=${SUPABASE_PROJECT_REF}`, and doc 11 states the policy that implements: read-only, scoped by default, write access granted per task and then removed. The scoping is real and is being honoured. What was never DECIDED is what it is scoped TO — `SUPABASE_PROJECT_REF` resolves to the production project `rloztdhzfliyvpvxsgjl`, held as a single Vercel row scoped 'Preview, Production' (SL-049's evidence). The default target was inherited from there having been exactly one project to point at; it was never chosen, so it was never reviewed.\n\nTHREE CONSEQUENCES, worth stating separately because they have different fixes. (1) Every session in every worktree that reads the database reads PRODUCTION, including the great majority doing work that has nothing to do with customer rows. (2) `--read-only` is the entire safety margin, and it lives in a checked-in argument list — removing it is a one-word edit with no second gate behind it, and the same variable feeds anything else that resolves a project ref. (3) The R-01 guard REFUSES to let an automated test touch this database while a human session is pointed straight at it with no equivalent check — the same asymmetry SL-049 recorded about preview links, reappearing in a different tool. A guard that binds only the actor who cannot make a judgement call is not a control.\n\nWHY NOW: a staging project exists as of 2026-08-11, ref `yoxmzwkxweasfaahhvpj` (recorded on SL-043, reported in-session and not independently verified). Re-pointing the session tool is the SMALLEST piece of SL-043 and does not wait on the Vercel variable split, the migration replay or the seed — it is one environment value. Keep `--read-only` afterwards regardless: the point is not that read-only stops being needed, it is that the mistake it fails to stop lands on a rehearsal copy.\n\nDONE WHEN a working session's database tool provably cannot read production — verified by reading a row that exists only in staging, which is the evidence standard SL-043 already sets, and not by inspecting configuration.\n\nTWO THINGS ADDED 2026-08-11, and together they make this worse than a wrong default. (1) NO TOOL TAKES A PROJECT PARAMETER. Every tool the Supabase MCP exposes — execute_sql, list_tables, query_logs, get_advisors, list_migrations — is bound at the PROCESS level by `--project-ref` in `.mcp.json`, and not one of them accepts a project argument per call. So an instruction reading 'staging only' cannot be followed by anyone, however careful: there is no argument to honour and no way to express the intent at the call site. The control cannot live in a prompt, a policy, a CLAUDE.md line or an agent's discipline — only in how the process is launched. That is precisely what makes this a PROCESS defect and not a matter of care, and it is why the fix is one environment value rather than a rule anyone has to remember. (2) IT IS CURRENTLY UNAUTHORIZED, WHICH IS LUCK RATHER THAN DESIGN. `apps/web/REQUESTS.md` records the Supabase MCP as unauthorized in-session, and that is the only reason the binding has not been exercised recently. Nothing chose it — a token expired. It re-authorizes straight back to production the moment anyone runs /mcp, with no prompt, no confirmation and no diff to review, so the present safety is an accident that will end quietly and without anyone deciding that it should.",
  },
  {
    roadmap: null,
    title:
      'Keep a copy of the replies we send, and let the record name every platform we can reply on',
    plain:
      'A reply sent from the inbox reaches the customer and is never written down on our side. The place to keep it was built and nothing writes to it, and two of the three platforms a reply can be sent on cannot even be named there. Why it matters: the shop owner cannot see what they already said without opening the platform itself, and a second person on the team can send the same reply again without ever knowing the first one went.',
    technical:
      "Owed to wt-db. Nothing here writes a migration, and nothing here should.\n\nWHAT IS MISSING. `apps/web/src/lib/inbox/send.ts` sends through Zernio and returns a receipt (`{status:'sent', platformId}`); no INSERT follows it anywhere on the path. `inbox_messages` was designed for exactly this row — `platform_message_id` alongside `sent_at`, under `check ((sent_at is null) = (platform_message_id is null))` so that a reply drafted here and a reply that reached the platform are different facts — and the migration's own comment calls the platform id 'the ONLY evidence it exists off our own database'. The table is right. Nothing writes to it.\n\nTHE VOCABULARY GAP, which is the part that needs wt-db rather than a feature lane. `SEND_WINDOWS` models three messaging platforms — facebook, instagram and whatsapp. The database can express one of them. FOUR sites carry the same narrow four-value list ('x','gbp','linkedin','instagram'): the connections platform CHECK (20260718000005, widened 20260801000001); `public.upsert_connection`'s guard (same widening migration); `public.upsert_zernio_connection`'s guard (20260804010000); and `inbox_threads.channel`'s CHECK (20260804020000).\n\nTHE FOURTH IS THE ONE THAT ACTUALLY BLOCKS THE WRITE, and it is easy to miss while the card is read as being about connections: a sent message hangs off a THREAD row, so a Facebook reply has nowhere to hang even if a Facebook connection were storable. `ChannelSchema` and `ConnectionPlatformSchema` carry the same four values and are the TypeScript half, which cannot be transactional with the SQL and is a frozen contract besides.\n\nWHY THE TWO VOCABULARIES DIVERGE, and why this is not 'add two strings'. The 2026-08-01 widening migration's header is explicit that the table CHECK and the RPC guard must move in ONE transaction, and equally explicit about why it stopped at instagram: widening the CONNECTION vocabulary past the CHANNEL vocabulary would let us store a connection for a channel the product cannot address. The inbox breaks that symmetry legitimately and says so — `send-window.ts` types `InboxPlatform` as EIGHT platforms and notes in as many words that it is 'Not the publishable Channel set'. We can reply somewhere we cannot publish. So the decision owed here is not two strings: it is whether `inbox_threads.channel` should keep borrowing the publishable set at all, or carry its own inbox vocabulary. Borrowing is what produced this.\n\nONE NOTE FOR WHOEVER WRITES THE INSERT. `inbox_messages.direction` is checked against ('inbound','outbound') while Zernio's wire words are `incoming` and `outgoing`. The mapping already exists as `messageDirection` and must be used rather than passing the wire value through — written straight into the column it is a constraint violation, and read straight out it was wt-inbox3's shipped defect, where every message in a thread rendered as the shop owner's own words.",
  },
  {
    roadmap: null,
    title: 'The dev dashboard stopped receiving updates because it is pointed at a dead address',
    plain:
      'Every update to the board, the changelog and the testing record is sent to an address that no longer answers, so the dashboard has been showing an older picture while the real record kept moving. Nothing has been lost — the updates queue and go through the moment the address works. Why it matters: a dashboard that is quietly out of date is worse than no dashboard, because it is still believed.',
    technical:
      "Found 2026-08-11 while filing SL-078 and SL-079. Every board move, changelog entry and QA run ends in `sync()`, which posts to `PRODUCTION_INGEST_URL` — `https://app.sahodalabs.com`, the default in `scripts/lib/ops-env.mjs`. That host returns 404 for `/api/admin/devops/ingest`.\n\nTHE ROUTE IS NOT MISSING FROM THE CODEBASE, which is what makes the 404 misleading: `apps/web/src/app/api/admin/devops/ingest/route.ts` exists, has `route.test.ts` beside it, and is in the deployed build. CAUSE, reported by DIVAS 2026-08-11: `app.sahodalabs.com` was never released from the OLD Vercel account and does not serve this project, so the name resolves to something that has never had this route at any version. It is the same two-accounts split that leaves the production project under `development-4417s-projects` while other tooling on this machine authenticates to `idivasms-projects`.\n\nWHY IT SURVIVED SO LONG: the failure is deliberately non-blocking, and correctly so. `ops-card.mjs` swallows it, `ops-sync.mjs` exits 0, and the message it prints ends 'Nothing was lost: state files keep their queue and replay once this is fixed' — which is TRUE, and is exactly why nobody ever acted on it. The state files in git stayed correct throughout; only the `ops_*` tables the console reads went stale. Observed concretely on 2026-08-11: SL-078, SL-079 and SL-070's move to done were written, committed, pushed and deployed while the dashboard showed none of them.\n\nFIX — one of two, and it is a decision rather than a repair. (a) POINT IT AT THE DOMAIN THAT WORKS: change `PRODUCTION_INGEST_URL` to the host actually serving this project, which is the alias production is already served on. One line, no infrastructure, verifiable immediately by making a board move and reading it back. (b) RELEASE THE HOST from the old Vercel account and attach `app.sahodalabs.com` to this project. Slower, needs access to an account this machine cannot authenticate to, and is the right answer if that domain is meant to be the product's address rather than a leftover. (a) unblocks the dashboard today and does not foreclose (b).\n\nDELIBERATELY NOT DONE HERE. `ops/.local.env` is gitignored, so a local override would fix one worktree and nobody else's — the exact shape of problem that lets a defect look fixed. Re-pointing the shared default and moving a domain are both infrastructure decisions rather than a lane's to take.\n\nDONE WHEN a board move made from a worktree appears on the dashboard, verified by making one and reading it back — NOT by the sync command exiting 0, which it already does today while failing.",
  },
  {
    roadmap: null,
    title: 'Subscribe only to updates on the live post feed, never to every change',
    plain:
      'When the posts screen starts listening for live changes, it must listen for updates only. Listening for everything would also deliver deletions, and a deletion carries nothing the database can use to check who is allowed to see it — so one customer could be handed the identifier of another customer’s deleted post. Why it matters: it is a leak between customers that no database rule can catch, and it costs nothing to avoid while the code is still unwritten.',
    technical:
      "[DOC] — reported by DIVAS 2026-08-11, sourced from Supabase's documented Realtime behaviour and NOT observed against this database. Re-tier to [LIVE] the first time a subscription runs and a delete event is actually watched.\n\nTHE RULE: subscribe with `event: 'UPDATE'`. Never `'*'`.\n\nWHY. Row-level security is NOT applied to DELETE events, and delete events cannot be filtered — the old row is not available for a policy to be evaluated against, so Realtime cannot decide who may see it and does not try. A `'*'` subscription therefore delivers every deletion in the publication to every subscriber, carrying the bare primary key. For `posts` and `post_variants` that is a post UUID belonging to another workspace. It is small, and it is still a cross-tenant disclosure — and the identifier is the very thing every other surface in this codebase treats as proof that a row is real.\n\nA MIGRATION CANNOT ENFORCE THIS, which is the entire reason it is a card rather than a line in a migration. `20260811000000_realtime_publish_state.sql` adds both tables to the publication and the existing member policies are right for INSERT and UPDATE; the DELETE gap lives in the Realtime layer on the SUBSCRIBER side, where no constraint, policy or trigger reaches. The only durable enforcement available is a TEST asserting the subscription's event filter is exactly `'UPDATE'` — the same shape as `sweeps`/`wiring.test.ts`, which exists because three files had to agree and every pairing had a silent failure mode. Without that test this card is a sentence someone has to remember, and the first `'*'` written in a hurry will pass every gate in the repo.\n\nWHERE IT LANDS: `apps/web/src/components/posts/live/publish-state-provider.tsx`. NOTHING THERE SUBSCRIBES TODAY — that file polls `readPublishState`, and grep finds no `postgres_changes`, no `.channel(` and no `subscribe(` anywhere in it. The publication migration is written and not applied, and there is still no authenticated browser Supabase client. So this is a rule for code that does not exist yet, filed at the cheapest moment available: before it is written, rather than after a leak.\n\nNOT A REASON TO DELAY THE SUBSCRIPTION. `'UPDATE'` is all the posts and planner surfaces need — they read status, scheduled_at, publish_status, permalink and platform_post_id off the NEW row, which is exactly what `apps/web/REQUESTS.md` asks wt-db for and why it explicitly does not request `replica identity full`. A post disappearing is a navigation rather than a poll result, as the provider says in as many words.",
  },
  {
    roadmap: null,
    title: 'Work out what actually published in one place, not two',
    plain:
      'The home screen decides what has been published by reading a different record from the one the rest of the app now uses, and the two can disagree. Three of these separate readings were just merged into one on purpose; this fourth was left behind because it sits on a different screen. Why it matters: two screens can tell the same person different things about whether their post went out, and nothing on either screen says which one to believe.',
    technical:
      "Filed 2026-08-13 by wt-web, immediately after the wt-status merge that created the rule this breaks.\n\nTHE RULE JUST ESTABLISHED. `apps/web/src/lib/posts/publish-evidence.ts` now owns the reading of 'did this go out', derived from `post_variants.publish_status`, for every per-post consumer. It replaced three independent derivations that had each drifted: `autoPublishTruth` told four production posts 'nothing was published' while their own channel chips two inches above read 'published'; `LiveStatusBadge` drew a post that was live on every channel as merely `committed`; and the week grid had no variant data at all. `listPublishModes` — which derived 'simulated' from `post_publish_logs.mode` — was DELETED rather than kept in parallel, and the LEARNINGS entry states the principle in four words: one fact, one derivation.\n\nWHAT WAS MISSED. `apps/web/src/lib/home/publishing.ts`'s `readPublishSummary` is the surviving instance of exactly the derivation that was deleted. It reads `post_publish_logs`, counts `status === 'succeeded'`, and splits `mode === 'live'` from `mode === 'fixture'` — the same live-vs-simulated fact, off the same table, that `listPublishModes` was removed for deriving. It survived the sweep because it is workspace-wide and feeds Home, so it matched none of the per-post call sites that were being fixed. `apps/web/src/lib/analytics/post-metrics.ts` also reads that table, but for metrics rather than for outcome, so it is not a fourth reading of the same fact and is out of scope here.\n\nWHY THE TWO CAN DISAGREE, concretely, rather than in principle. They read different tables written by different actors. `post_variants` is written by the publish path; `post_publish_logs` is service-role-insert and apps/web never writes it, so only apps/jobs produces rows. A variant can therefore be `published` with no log row — and `readPublishSummary` would report the workspace as having published nothing while the post screen shows it published. The reverse also holds through the cap: the read is `LIMIT 200` oldest-first-truncated, so a busy workspace's most recent publishes fall out of the count entirely while the per-post reading still shows them.\n\nNOT A BUG TODAY, AND THAT IS THE POINT OF FILING IT NOW. Both readings are individually correct about their own table, and Home is honest about its limits — it separates `empty` from `unreadable`, states `capped`, and counts modes only on success. Nothing is currently known to be wrong on screen. This is the same shape as the three defects wt-status fixed: each was individually reasonable and only wrong against the others, and none was noticed until a customer's own screen contradicted itself.\n\nDONE WHEN Home's summary derives from the same source as `publishEvidence`, or when a comment on `readPublishSummary` states in the file why a second derivation is correct here and what keeps the two consistent. Either resolves it; leaving both readings unremarked does not.",
  },
  {
    roadmap: null,
    title: 'A guided tour still offers to build and publish a website we have hidden',
    plain:
      'One of the guided tours tells the customer to describe a website and we will build it and put it online. Putting it online was never finished, and the whole section has now been taken out of the menu for the beta — so the tour makes a promise, about a screen the customer cannot reach, for something that does not work. Why it matters: it is a promise of a capability we do not have, which is the one thing this product refuses to do.',
    technical:
      "Filed 2026-08-13 by wt-web, alongside the change that hid the section.\n\nTHE TEXT. The seeded tour `site.first_tour` triggers on first visit to `/sites` and says, verbatim: \"Describe a site and I'll build + deploy it.\" Its step targets the anchor `sites.generate`.\n\nBOTH HALVES ARE NOW WRONG, and they became wrong for different reasons on different days, which is why neither was caught. (1) DEPLOY WAS NEVER BUILT. Generation and in-app preview work; the rest of the chain — real address, contact form, leads — is unowned. So 'build + deploy' overstated the product from the day it was seeded. (2) THE ROUTE IS NOW HIDDEN. The module was descoped from the beta on 2026-08-13 and removed from the nav rather than shown half-finished, which is recorded on the Alpha record as an out-of-scope decision. The tour still fires on first visit to a route reachable only by typing it.\n\nWHY THIS IS A CARD AND NOT AN EDIT. The tour lives in `packages/db/supabase/migrations/20260718000010_seed.sql`, an APPLIED migration. Applied migrations are immutable and only wt-db writes them — both are non-negotiables. There is no fix any other lane can make.\n\nTHE REMEDY IS A NEW ROW, NOT AN UPDATE, and this is the part easiest to get wrong. Tours are versioned: the seeded row is `version: 1`, keyed by (id, locale, version). Editing the existing row in place would rewrite history for anyone who already saw v1 and would leave the two copies of the tour — the migration's and the database's — permanently disagreeing, which is the drift the anchor integrity test exists to catch elsewhere. wt-db should insert a `version: 2` row for `site.first_tour`.\n\nTHREE OPTIONS, in order of preference. (a) RETIRE IT while the module is out of scope — a tour for an unreachable screen has no correct copy. (b) Re-word v2 to promise only what exists: describe a site and see it built, with no claim about publishing. (c) Leave it and accept that a customer who types the URL is told we will put their site online. (c) is recorded only to be rejected explicitly.\n\nNOTE FOR WHOEVER TAKES IT: `sites.generate` IS rendered, so this tour is not caught by the missing-anchor path. The step will fire and spotlight a working generate control, then make a claim about deploying that the control does not do — which is worse than an auto-skip, not better. A missing anchor degrades silently and truthfully; this one works and lies.\n\nDONE WHEN the tour a first-time visitor to that route receives either does not exist or makes no claim about publishing — verified by reading the row the app actually serves, not the migration.",
  },
  {
    roadmap: null,
    title: 'Recording one test by hand can stop the whole dashboard updating',
    plain:
      'The testing record keeps its most recent two hundred runs, and the dashboard refuses updates that carry more than two hundred. Those two numbers are the same, so the automatic record sits permanently full — and adding a single test result by hand, which is the way we are told to record manual testing, pushes it one over and the update is refused outright. Why it matters: the board, the changelog and the testing record travel together, so one extra test result silently stops all three, and nothing on the dashboard says why.',
    technical:
      "Found 2026-08-14 by wt-ingest while fixing SL-080, and found only because a guard was added — the failure is invisible to every other check in the repo.\n\nTHE TWO NUMBERS. `OpsIngestPayloadSchema` caps `qa` at `.max(200)` (packages/shared/src/ops/state.ts). `ops-hook-bash.mjs:91` trims the queue with `.slice(-200)`. Identical ceilings, zero headroom: the automatic path parks the queue at exactly the limit forever.\n\nTHE TIP-OVER. `/qa-log` and any hand-written entry in `qa.pending.json` APPEND without re-applying the slice, so the queue reaches 201. The endpoint parses with `.strict()` over the whole object, so the response is not 'your QA is too long' — it is a flat rejection of the entire payload. Board moves and changelog entries in that same sync are refused too, having nothing wrong with them.\n\nOBSERVED, not reasoned: adding one manual QA row for SL-080 produced `qa: Too big: expected array to have <=200 items` from the real schema. It was caught by `apps/web/src/lib/ops/state-sendable.test.ts`, added in the same change. It did NOT surface during the replay, because that drove the `ops_ingest` RPC directly and the RPC has no zod in front of it — a useful warning about that shortcut in its own right.\n\nWHY IT HAS NEVER BEEN SEEN. It needs a full auto queue AND a manual entry AND a reachable server. For a week there was no reachable server, so every sync failed at the transport for a different reason and no payload was ever validated. The three conditions first coincided the day the address was corrected.\n\nFIX — one of three, and it is a judgement rather than a repair. (a) MOVE THE CAP TO THE WRITER: apply the trim in `ops-state.mjs` where anything appends, so no caller can exceed it. Closes it for every appender including ones not yet written, and is the only option that does not depend on remembering. (b) GIVE THE SCHEMA HEADROOM: raise the payload cap above the queue trim so the two are not equal. One character, and it leaves two numbers that must stay in a relationship nothing enforces. (c) TRIM AT SEND in `ops-sync.mjs`. Rejected: it drops a QA run that was never delivered, which is the same silent loss recorded below.\n\nRELATED LOSS, worth reading together. The `.slice(-200)` is not only a ceiling, it is an eviction. While the sync pointed at a dead host, 140 QA runs committed at abaf161 were dropped from the queue before it could ever drain — confirmed by comparing the committed file with the working tree. SL-080's card says 'Nothing has been lost — the updates queue and go through the moment the address works.' That is TRUE of the board and the changelog, whose queues are uncapped. It is FALSE of QA. The queue is a ring buffer, and a ring buffer pointed at a dead endpoint is a shredder.\n\nDONE WHEN a manual QA run can be recorded on top of a full automatic queue and the next sync is accepted — verified by doing it and reading the row back, not by the payload happening to be short that day.",
  },
]
