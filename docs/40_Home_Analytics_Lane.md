...
## 5 · What the guards can and cannot see

See §7 of this file after implementation, and the header of
`apps/web/e2e/helpers/accent-spend.ts`, which carries the instrument's own limits.

One limit belongs here because it changes how the table in §1.1 must be read:
**`regions` is comparable within a theme, not across themes.** The same route, the
same width, the same data state and therefore the same DOM measures 15 regions in
light and 30 in dark (`/home` 1440 empty) for nearly the same pixel count — 1392
against 1758. `--acc` is `#bd4b00` in light and `#ff6600` in dark, and the two
antialias across the s>0.30 floor differently against their own grounds. A
light-to-dark regions delta is an artefact; a same-theme before-to-after delta is a
measurement.
