...
| --- | --- | --- | --- | --- |
| **Firecrawl** | The Brand Brain cannot read a customer's website in onboarding (`packages/research` uses it in three places); the flow still finishes from the answers alone | `FIRECRAWL_API_KEY` (`FIRECRAWL_API_URL` only if self-hosted) | firecrawl.dev → API keys | Free tier 500 pages; 50 onboardings × ~10 pages fits. $16/month for the next tier |
...
| **Zyte / Apify** (Radar competitor scans) | Radar's vendor question (old doc, Part 4) is unanswered; `RADAR_FIXTURES` stays set so the page shows fixtures labelled as such | `ZYTE_API_KEY`, `APIFY_TOKEN`, `SAHODA_RADAR_SCAN_MODE` |
