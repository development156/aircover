...
- **The `@smoke` leg runs on CI against staging and PASSES: 122 of 122, 0 skipped, run 34021699957 on `973c6534` (2026-09-06, 14:10 IST), three shards, none near the 60-minute limit.** It took nine dispatches to get there. Three were blocked by secrets (production values, then missing, then a re-saved URL); one reached assertions and was killed by the time limit; the first complete run (34012814133) passed 107 and its 15 failures were read from the traces and the database's own log; the wrong staging password in `E2E_SUPABASE_DB_URL` (104 pooler auth failures) hid behind "missing heading" until `scripts/smoke-db-probe.mjs` named it; once the founder reset it, 116, then 120, then 122. Every fix is a commit in the findings table or the handoff; no ceiling was raised to pass and no test was deleted.
...
1. ~~Reset the staging database password~~ **DONE by the founder 2026-09-06 11:58 IST**; the probe went green on the next dispatch and the suite passed 122/122 two runs later.
