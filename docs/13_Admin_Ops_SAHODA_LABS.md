...
```
Rules: approver must be a different active `admin`/`owner` than the requester (**superseded 2026-07-27**: the env var `OPS_ALLOW_SELF_APPROVE` is gone. It was passed to the RPC as `p_allow_self`, and because `ops_credit_request_verify` is granted to `authenticated`, PostgREST exposed that parameter to the caller — a direct `supabase.rpc()` set it `true` itself and skipped the approver check, so any writer-role admin holding a valid code could grant. Proven exploitable, fixed in migrations 16–17. Self-approve is now the Postgres setting `sahoda.allow_self_approve`, which a PostgREST client has no surface to write: `alter database postgres set sahoda.allow_self_approve = 'true'` on a dev project only. Unset everywhere today. Still stamps `self_approved`); grant goes through `apply_ledger_entry()` exactly once (idempotency key = request id — a retry can never double-grant); `action_type='admin_grant'`, `object_ref=request.id`, visible in the customer's wallet "why" as "Credits added by Sahoda Labs team."; workspace picker = server-side search by name/slug/owner-email (service role, read-only); amounts capped at 10,000/request; everything audit-logged. Negative adjustments are out of v1 (do them in SQL with two humans present).
...
- **JSON export/import:** export = `{schema:'sahoda_qa_v1', exported_at, runs:[…], artifacts:[…meta only…]}` (zod-validated). Import validates against the same schema; upserts by id; conflicts and rejects listed in a result panel, never silently merged. This is the portable QA record the founder asked for.
  - **Round-trip means *the same runs, re-attributed* — not byte-identical.** An imported run is filed under whoever imported it, `kind` is forced to `manual`, and the author named in the file is preserved as a *claim* in `details` ("Imported by X; the file attributed this run to Y"). Everything else — id, task, suite, status, summary, timings — round-trips unchanged, so a wipe-and-restore returns the same record with honest provenance.
  - **Why (2026-07-27, approved):** the original `ops_qa_import` did `coalesce(rec.actor, caller)`, so the payload chose the author. Any writer could file `pass` rows for suites that never ran, attributed to a colleague. Nobody reads the audit log to decide whether a build is healthy — they read this console, and it is the source for gate-health over time (§12, charts). Faithful `actor` round-tripping and non-repudiation of the QA record are in **direct conflict**, and non-repudiation wins: the column that answers "who put this in the record" must always answer truthfully. Fixed in migration 18; pinned by `packages/db/tests/ops_qa_import.test.ts`.
...
**Decision (2026-07-26, approved): `/admin` lives OUTSIDE the `(app)` route group — do not "fix" this.** It is `app/admin/`, not `app/(app)/admin/`, so it does not inherit that layout. `(app)/layout.tsx` renders `Topbar`, which reads the workspace list, the active workspace slug and a credit balance — all tenant state. An ops admin is a **platform** role and need not own a workspace at all, so inheriting that layout would make the console depend on something it has no reason to have, and would break for a legitimate admin with no workspace. `/admin` therefore ships its own header (title · role chip · `UserButton`) and keeps only `Rail`, because an admin is also a user and needs the way back out. This is the same reasoning as "ignores workspace theming by design" above, applied one level up: the console is not a tenant surface.
...
☐ Non-admin (and anon) hits `/admin/*` → 404; anon RLS suite green on all `ops_*` ☐ Restricted signup: uninvited email cannot register; approve → invitation → signup works → row `joined` via webhook ☐ Embed form on a foreign origin submits → row appears in inbox in realtime; Turnstile fail + rate-limit paths rejected ☐ Credit flow: A requests, B's OTP approves → wallet +N with correct "why"; replay of same request grants **nothing**; wrong-code ×3 expires; a third admin holding the right code grants **nothing**; self-approve off unless the database says otherwise (`sahoda.allow_self_approve`, unsettable by any client) ☐ A real Claude session: task moves todo→in_progress→review→done on the live board **without any human touching the UI**; commit with `SL-###` auto-lands Done with sha ☐ Gates strip mirrors a real failing test as red, then green after fix ☐ Changelog authors run DIVAS→GIRIJA→DIVAS AND GIRIJA over three consecutive entries; copy buttons yield clean plain+md ☐ Manual QA run with pasted screenshot survives a tab kill via autosave; JSON export→wipe→import restores the same runs, re-attributed per §11 (id/task/suite/status/summary/timings identical; `actor` = the importer, `kind` = `manual`, original author kept as a claim in `details`) ☐ Roadmap card % , days-remaining, and "To reach Done" all recompute after a task completes ☐ `turbo typecheck lint test` + smoke green · no raw hex · no new table without RLS test · LEARNINGS line added.
...
`docs/13_Admin_Ops_SAHODA_LABS.md` (this file) + 00_README row · migrations + `packages/shared/ops.ts` · `scripts/ops-sync.mjs` + `ops-hook-bash.mjs` + `ops/state/*` seeds · `.claude/skills/sahoda-devops/` + sahoda-db amendment (§3) · commands `/task /qa-log /log-change` + `/ship` extension · settings.json hook entries (§9.3) · `.env.example` += `DEVOPS_INGEST_TOKEN= · ADMIN_BOOTSTRAP_EMAILS=` (**not** `OPS_ALLOW_SELF_APPROVE` — removed 2026-07-27; see §6. An env var cannot gate a decision the client gets to make, and leaving it declared would be a control that silently does nothing) · CLAUDE.md += one line: *"You are also PM + QA Lead: follow sahoda-devops in every session; the /admin board, changelog and QA console must reflect reality at all times."*
