...
> **Numbering clash, for whoever integrates this.** The `wt-ops` lane already
> holds `docs/37_Tenant_Isolation.md`. The filenames differ, so git merges both
> without a conflict — but two documents numbered 37 breaks the canon order in
> `docs/00_README`. Renumber one deliberately at integration. This file keeps the
> number the brief assigned it rather than picking a new one silently.
