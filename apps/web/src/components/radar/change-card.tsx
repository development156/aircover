    ...
    >
      ...
      {change.reading ? (
        <div
          ...
          // The rung at panel scale, echoing the chip. tokens.css requires the
          // word, and `ReadMark` carries it — the class never appears here
          // without that chip beside it.
          //
          // `is-proposed`, not `is-simulated`: docs/26 §3.1b rules that the
          // fourth rung means NOT REAL (a fixture), and a Radar reading is
          // derived from snapshots that were actually taken. This is the echo of
          // the chip in `marks.tsx`, so the two must never disagree — the pair
          // is asserted together in `marks.rung.test.tsx`.
          className="is-proposed flex flex-col gap-2 rounded-card p-3"
        >
          <div className="flex items-start gap-2">
