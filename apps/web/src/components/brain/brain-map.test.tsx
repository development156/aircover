...
import { DORMANT_STATES, type MapStates } from '@/lib/brand/brain-map'
...
describe('BrainMap', () => {
  test('draws one node per field, each carrying its state', () => {
    const { container } = render(
      <BrainMap
        states={states({ 'hook.core_promise': 'confirmed', 'taboo.red_lines': 'intake' })}
      />,
    )
  ...
  test('speaks the numbers, never the picture', () => {
    render(<BrainMap states={states({ 'hook.core_promise': 'confirmed' })} />)
    expect(screen.getByRole('img')).toHaveAccessibleName(/1 of 15 fields confirmed/)
  })

  test('a page arriving does not pulse', () => {
    const { container } = render(<BrainMap states={states({ 'hook.core_promise': 'confirmed' })} />)
  ...
  test('the node that just became confirmed lights, and only that one, then settles', () => {
    const { container, rerender } = render(<BrainMap states={states()} />)

    rerender(<BrainMap states={states({ 'hook.core_promise': 'confirmed' })} />)
  ...
  test('a field moving to intake does not pulse: nothing was confirmed', () => {
    const { container, rerender } = render(<BrainMap states={states()} />)
    rerender(<BrainMap states={states({ 'taboo.red_lines': 'intake' })} />)
    expect(container.querySelector('[data-lit="true"]')).toBeNull()
  })

  test('dormant draws the frame with no arc and says so', () => {
    const { container } = render(<BrainMap states={DORMANT_STATES} dormant />)
