...
export type StoreEmptinessState =
  ...
  | 'store_unreadable'
  /**
   * The COUNT of connected accounts could not be taken. Distinct from
   * 'never_connected', which is a claim that there are none — this is the
   * absence of a measurement, and it exists because a failed count arriving as
   * a zero would render that claim.
   */
  | 'accounts_unreadable'
...
export interface StoreDecideInput {
  ...
  /**
   * Active connections Zernio could send events for. A measurement, never a
   * guess — and `null` when we could not take one.
   *
   * THREE-VALUED FOR THE SAME REASON `surface.ts` IS. `countAccounts` used to
   * `return 0` when its own query errored, and 0 is the input that decides
   * `never_connected` below: "No conversations yet … once you connect an
   * account." A failed count therefore told a customer with four channels
   * connected that they had none, and offered them a remedy they had already
   * carried out. That was fixed in the LIVE classifier; this store classifier
   * was written on a branch predating the fix and arrived with the same hole —
   * a thing only the merge of the two could see.
   */
...
export function decideStoreSurface(input: StoreDecideInput): StoreDecision {
  ...
  // Checked BEFORE `=== 0`, because the collapse this guards against is exactly
  // a null arriving where a zero is read. Same claim as `store_unreadable`
  // above: the attempt failed, so nothing is said about the customer.
  if (input.connectedAccounts === null) {
    return {
      state: 'accounts_unreadable',
      showList: false,
      headline: `Sahoda could not check your connected accounts`,
      body: `This is not a reading of your ${noun}. Sahoda could not confirm which accounts are connected, so it will not guess at what belongs here. Try again in a moment.`,
    }
  }
