...
describe('decideStoreSurface', () => {
  ...
  it('does NOT say "nothing connected" when the account COUNT is what failed', () => {
    /*
     * FOUND AT INTEGRATION, 2026-08-22, and it is the same defect twice.
     *
     * `countAccounts` used to `return 0` when its own query errored. Zero is the
     * input that decides `never_connected` — "No conversations yet … once you
     * connect an account" — so a failed count told a customer with four channels
     * connected that they had none, and offered them a remedy they had already
     * carried out. A sibling lane fixed that in the LIVE classifier and widened
     * the count to `number | null`.
     *
     * This classifier was written on a branch that predated the fix, narrowed the
     * value back to `number`, and DEFAULTED it to 0. Neither lane could see it:
     * one had no store classifier, the other had no nullable count.
     */
    const d = decideStoreSurface({ ...base, connectedAccounts: null, eventsEverReceived: true })
    expect(d.state).toBe('accounts_unreadable')
    expect(d.showList).toBe(false)
    // THE FORBIDDEN CLAIMS: that they have no conversations, and that connecting
    // an account is the remedy — they may well have connected several.
    expect(`${d.headline} ${d.body}`).not.toMatch(/\bno conversations\b/i)
    expect(`${d.headline} ${d.body}`).not.toMatch(/\bconnect an account\b/i)
    // And it says what actually happened, so the reader knows to try again.
    expect(`${d.headline} ${d.body}`).toMatch(/could not/i)
  })
