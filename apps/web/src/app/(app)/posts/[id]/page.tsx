...
export default async function PostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // this read existed (wt-assets). It survived the composer replacing PostEditor
  // only because it was looked for: `libraryNames` is an OPTIONAL prop, so the
  // composer not passing it would have cost the feature in silence rather than
  // failing to compile. Degrades to an empty map, which is the pre-library
  // behaviour, so a hiccup costs a nicer label and nothing else.
  const libraryNames =
    media.length > 0
      ? Object.fromEntries(await readLibraryNames(media.map((row) => row.storage_path)))
      : {}
